...
def parse_mnist(image_filename, label_filename):
    ...
    with (gzip.open(image_filename, 'rb') as f):
        ...
        X = np.divide(raw_X, 255, dtype=np.float32)
    with (gzip.open(label_filename, 'rb') as f):
        f.read(8)
        y = np.fromiter((num[0] for num in struct.iter_unpack("B", f.read())), dtype=np.uint8)
    return (X, y)
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for i in range(0, X.shape[0], batch):
        X_batch = X[i:(i + batch), :]
        y_batch = y[i:(i + batch)]
        temp = np.exp(X_batch @ theta)
        Z = temp / np.sum(temp, 1, keepdims=True)
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(0, X.shape[0], batch):
        ...
        I_y[np.arange(batch), y_batch] = 1
        temp2 = np.exp(Z1 @ W2)
        G2 = temp2 / np.sum(temp2, 1, keepdims=True) - I_y
